...
- [x] **C6 Rod/pipe cutting & offcut tracking** — DONE (migration `c6_cutting_offcuts`; `CuttingJob` + `CuttingOutput` + `CUT_OUT`/`CUT_IN` move types). Cut a full bar/pipe/sheet into the piece a customer needs + leftover offcuts. A cut is a **value-conserving stock conversion** (no money moves): the bar leaves (`CUT_OUT`), each piece/offcut re-enters (`CUT_IN`) at its share of the bar's weighted-avg cost (split by length, fallback qty), and its `costPrice` is folded like a purchase — so offcuts are valued on the books and a later sale carries the right COGS. Optional saw/scrap **wastage leaves as a `DAMAGE` loss** (real P&L hit — the schema's WASTAGE reason), keeping the sheet exact. No new integrity invariant (rounding absorbed by the existing revaluation term). New Cutting page (Inventory nav, Scissors). Gated `stock.adjust`. 25/25 checks — length-based cost split (₨300/₨100 from a ₨400 bar), offcut weighted-avg (₨200), wastage removes exactly its value as a loss, sold piece COGS = allocated cost (profit ₨700), guards reject over-cut & full-waste, integrity all-green + BS ₨0 throughout. **✅ Batch C COMPLETE (C1–C6).**
- [x] **Batch D — Inventory & purchasing (D1–D5)** — DONE (migration `d_inventory_purchasing`). **D1 Cost/price history:** `PriceHistory` log appended on product create, manual price edit, and each purchase's weighted-avg cost change; new *Cost / Price History* report (per-product trend). **D2 Negative-stock/backorder:** `allow_negative_stock` setting (Settings → Shop Profile) lets a sale oversell — stock goes negative and the line is flagged `backordered`; new *Backorders* report; off by default (ledger still blocks over-issue → 409). **D3 Barcode/shelf-label print:** dependency-free Code128 encoder + a *Print Labels* page (pick products, copies, columns, name/price/SKU/barcode toggles, browser print). **D4 Vendor debit/credit notes:** `VendorNote` (CREDIT lowers payable = income, DEBIT raises it = cost) adjusts the vendor balance; the counterpart flows to retained earnings in the balance sheet and the vendor-reconciliation check includes notes — books stay exact. **D5 Purchase Orders → GRN:** `PurchaseOrder`/`PurchaseOrderItem` (non-financial); receiving against a PO creates a normal RECEIVED `Purchase` (stock in + weighted-avg + payable) and advances the PO to PARTIAL/RECEIVED — partial receipts supported, no new balance-sheet term. New pages: Purchase Orders, Vendor Notes, Print Labels. 24/24 checks — oversell → stock −3 + backorder flag + report + 409 when off; credit/debit notes reshape payable with BS counterpart + reversal; PO partial (₨200) then full receipt raising payable + over-receipt rejected; price-history rows; **integrity all-green + BS ₨0 throughout**. Both apps tsc clean. **✅ Batch D COMPLETE.**
- [x] **Batch E — Customer engagement & messaging (E1–E4)** — DONE (migration `e_messaging`). **E1** auto-emailed monthly statements (PDF via shared exporter; manual + monthly cron). **E2** local SMS gateway 🔌 (provider-agnostic HTTP sender configured in Settings → Integrations — owner fills URL/key). **E3** bulk greetings (SMS/email server-side + WhatsApp wa.me links). **E4** tiered udhaar reminder escalation (gentle→firm→final by debt age, per-customer tier + cooldown, daily sweep). New Outreach page + customer email. Messaging writes nothing to the ledgers → integrity inherently untouched. 11/11 checks (escalation/cooldown, campaign links, graceful no-SMTP, **integrity all-green + BS ₨0**). Both apps tsc clean. **✅ Batch E COMPLETE.**
- [x] **Batch F — HR & staff (F1–F3)** — DONE (migration `f_hr`). **F1** payslip PDF from a salary payment (shared exporter; download on the Salaries tab). **F2** biometric attendance CSV import (flexible columns — employee code + date + status or in/out punches — upserts marks; upload on the Attendance tab). **F3** salesman commission (`User.commissionPercent` + a per-rep commission report on net sales; % field on the Users form). All read-only/no-money except salary (pre-existing, proven). 7/7 checks (commission ₨50, payslip + PDF, attendance import, **integrity all-green + BS ₨0**). Both apps tsc clean. **✅ Batch F COMPLETE.**
- [x] **Batch G — POS experience (G1–G5)** — DONE (migration `g_pos`). **G1** quick-sale favourites (star tiles + strip in POS, `pos_favourites` setting). **G2** walk-in return with no invoice (`POST /sales/blank-return` — stock back in + cash refund, own page). **G3** discount approval (`max_discount_percent` cap → 409 unless a manager approves in POS). **G4** loyalty points (`Customer.loyaltyPoints` + `LoyaltyEntry`, earn per ₨100 / redeem as a bill discount — a memo, no booked liability, so no balance-sheet surface). **G5** customer display (`/pos/display` mirrors the cart live via BroadcastChannel). 12/12 checks (loyalty earn/redeem, discount gate+override, walk-in return stock+2, favourites, **integrity all-green + BS ₨0**). Both apps tsc clean. **✅ Batch G COMPLETE.**
- [x] **Batch H — Platform & safety (H1–H7)** — DONE (migration `h_platform`). **H1** offsite cloud backup 🔌 (nightly PUT of the JSON snapshot to an owner-supplied URL). **H2** saved report filter presets (per user). **H3** TOTP 2FA (dependency-free RFC 6238; login challenge + Settings → Security). **H4** import-purchase FX (buy in USD/AED at a rate; books stay PKR). **H5** GST/tax register (output − input = net). **H6** bank reconciliation (mark/import-match ledger entries cleared — memo only). **H7** offline POS (localStorage queue + sync; server dedupes by `clientRef`). 18/18 checks (real 2FA, FX ₨2,800, GST net ₨833, reconcile memo, offline no-dup, saved filters, **integrity all-green + BS ₨0**). Both apps tsc clean. **🔌 H1 needs the owner's upload URL. ✅ Batch H COMPLETE — ALL 36 FEATURES (A–H) DONE.**
